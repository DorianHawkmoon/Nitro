...
TEST(NAME_CLASS, Constructor) {
	...
	EXPECT_STRCASEEQ(inter.ToString().c_str(), "No intersection");
	EXPECT_STRCASEEQ(inter2.ToString().c_str(), "Between rectangle 2 and 3 at (10,15), w=20, h=30");
}
...
TEST(CLASS_NAME, GetIntersectionShape) {
	...
	EXPECT_STRCASEEQ(shape->ToString().c_str(), "Rectangle at (10,15), w=20, h=30");
...
TEST(NAME_CLASS, AddIntersection) {
	...
	EXPECT_STRCASEEQ(added->ToString().c_str(), "Between rectangle 2, 3 and 5 at (5,3), w=5, h=5");
	EXPECT_STRCASEEQ(added2->ToString().c_str(), "Between rectangle 2, 3, 5 and 4 at (5,3), w=3, h=5");
...
TEST(NAME_CLASS, Strings) {
	...
	EXPECT_STRCASEEQ(added2->ToString().c_str(), "Between rectangle 2, 3, 5 and 11 at (5,3), w=5, h=5");
}